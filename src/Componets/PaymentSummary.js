...
function PaymentForm() {
    ...
    useEffect(() => {
        ...
        axios.get("http://localhost:5000/Ohkla/getYear")
            .then((res) => {
                ...
                // useEffect(()=>{},[]) inside updated code of receiptCounter
                const storedCounter = localStorage.getItem("receiptCounter");
                const counter = storedCounter ? parseInt(storedCounter) : 101;
                localStorage.setItem("receiptCounter", counter.toString()); // Ensure it's stored
                setReceiptNo(`REC-${counter}`);
            })
    ...
    <div style={{ display: 'flex' }}>
     ...
     <div className="card shadow mt-0 p-1" style={{ width: '100%', height: '700px' }}>
        ...
        <h3 className="mb-0" style={{ textAlign: 'center' }}>
          ...
          <div className="row g-3">
            ...
            <InputSelect
              label={<><span>Payment Mode</span> <span className="star">*</span></>}
              value={paymentMode}
...
//     const fetchMember = async () => {
//         if (!selectedId || !paymentType || (paymentType === "Annual" && !selectedYear)) {
//             alert("⚠ Please select Company, Payment Type, and Year (only for Annual).");
//             return;
//         }
//         try {
//             setIsLoadingData(true);
...
//         if (paymentType === "Registration") {
//             if (!registrationFees || registrationFees <= 0) {
//                 alert("⚠ Registration Fees must be greater than 0.");
//                 return;
//             }
//         } else if (paymentType === "Other") {
//             if (!otherCharge || otherCharge <= 0) {
//                 alert("⚠ Please enter a valid Other Charge.");
//                 return;
//             }
//         }
